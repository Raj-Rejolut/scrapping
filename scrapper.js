...
const writeToFile = async (fileName, data) => {
    const filePath = `C://Users//DELL//Desktop//rates-web-scrapper//results//${fileName}`;
...
const bfcExchangeRateScrape = async () => {
    ...
    const rates = await page.evaluate(() => {
        return Object.values(document.body.querySelectorAll('div > table > tbody > tr'))
            .map((x) => ({
                currency_code:
                    x
...
//Pending It has currency name not currecny code names are not same as with different exchanges
...
const joyalukkasExchangeRateScrape = async () => {
    ...
    const rates = await page.evaluate(()=>Array.from(document.body.querySelectorAll('tr'),e=>{let r=e.querySelectorAll('td'),[l,t,o,n,u]=Array.from(r,e=>e.innerText);return{currency:l,currency_code:t,buy:o,cash_buy:n,cash_sell:u}}));
...
//     const baseCurrency = 'AED';
//     const { data } = await axios.get(`https://alfardanexchange.com/`);
//     const currencyCodes = [];
//     const axiosRequests = [];
...
const orientExchangeRateAPI = async () => {
    ...
    try {
        const res = await axios.post(
            apiCallURL,
            qs.stringify({ selLoc: 13, requestType: 'selectedLocIdVal' }),
            {
                headers: {
                    Cookie: orientExchangeRateAPICallData.request_headers_cookie,
                    'Content-Type':
                        orientExchangeRateAPICallData.request_headers_content_type,
                },
            }
        );
...
const alfardan2ExchangeRateScrape = async () => {
    ...
    console.log('Al Fardan rates extraction completed successfully!', { baseCurrency, rates });
};
...
const abusheikhExchangeRateAPI = async () => {
    ...
    const { data } = await axios.get(
        'https://abusheikhaex.com/news/currency-exchange-made-simple-4-ways-get-best-rate'
    );
    ...
    const rates = responses.map((res, i) => ({
        currency_code: currencyCodes[i],
        buy: res.data[`AED_${currencyCodes[i]}`] ?? 0,
    }));

    // await writeToFile('abusheikh.json', { baseCurrency, rates });
    console.log('Abusheikh rates extraction completed successfully!', { baseCurrency, rates });
};
abusheikhExchangeRateAPI();
